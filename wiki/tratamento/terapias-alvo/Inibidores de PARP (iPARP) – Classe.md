---
title: "Inibidores de PARP (iPARP) – Classe"
aliases:
  - "Inibidores de PARP (iPARP)"
category: "Terapias-Alvo"
source_documents:
  - "Metastático"
tags:
  - "terapia-alvo"
status: "draft"
---

# Inibidores de PARP (iPARP) – Classe

Os **inibidores de PARP (iPARP)** — [[Olaparibe|olaparibe]] e talazoparibe — são agentes orais que bloqueiam a enzima poli-ADP-ribose polimerase (PARP), envolvida no reparo de quebras de fita simples do DNA. Em tumores com deficiência pré-existente no reparo por recombinação homóloga (HRD), como os de pacientes com mutações germinativas em [[BRCA1 - BRCA2|BRCA1/2]], o bloqueio da PARP induz **letalidade sintética**, resultando em morte celular seletiva.

## Evidências

| Estudo | Cenário | Comparação | PFS | SG |
|--------|---------|------------|-----|-----|
| **[[OlympiAD]]** (NEJM 2017) | BRCA germinativo, metastático ≥ 1L QT | Olaparibe vs QT escolha | 7,0 vs 4,2m (HR 0,58) | 19,3 vs 17,1m (HR 0,89) |
| **[[EMBRACA]]** (NEJM 2017) | BRCA germinativo, metastático ≥ 1L QT | Talazoparibe vs QT escolha | 8,6 vs 5,6m (HR 0,54) | 19,6 vs 19,3m (HR 0,85) |
| **[[OlympiA]]** (Lancet 2021, SG 2023) | Adjuvante BRCA, alto risco | Olaparibe 1a vs placebo | SLD HR 0,58 | SG +3,4% em 4a (HR 0,68) |
| **[[TBCRC 048 (Olaparib Expanded)|TBCRC 048]]** (JCO 2021) | Germinativo PALB2+ metastático | Olaparibe | Taxa de resposta 82% | — |

## Mecanismo de Ação

A PARP-1 e PARP-2 são enzimas que detectam e reparam quebras de fita simples no DNA. Quando inibidas por um iPARP, essas lesões persistem e colapsam na forquilha de replicação, gerando quebras de fita dupla. Em células normais, estas são reparadas por recombinação homóloga (dependente de BRCA1/2 e PALB2). Na ausência de BRCA funcional (tumoral), o reparo é ineficaz, levando ao acúmulo de dano genômico e apoptose.

## Indicações em Câncer de Mama

**Adjuvante (doença localizada de alto risco)** : olaparibe por 1 ano para portadoras de mutação germinativa BRCA1/2 com [[Doença Residual|doença residual]] pós-neoadjuvância ou com tumor ≥ T2 ou N+ (OlympiA). Em 4 anos, o ganho absoluto em SG foi de 3,4% (89,8% vs 86,4%, HR 0,68). A indicação abrange tanto TNBC quanto [[Receptores Hormonais (RE-RP-RH)|RH]]+/[[HER2]]− ([[Sociedade Brasileira de Oncologia Clínica (SBOC)|SBOC]] 2026, NE ALTO/FR FORTE).

**Metastático**: olaparibe (OlympiAD, PFS 7,0 vs 4,2 meses, HR 0,58) e talazoparibe (EMBRACA, PFS 8,6 vs 5,6 meses, HR 0,54) são opções para pacientes com mutação germinativa BRCA1/2, após progressão a [[tratamento/quimioterapia/index|quimioterapia]] ou hormonioterapia. O olaparibe também é ativo em portadoras de mutação germinativa PALB2 (TBCRC 048). Ambos são bem tolerados, com fadiga, náusea e anemia como eventos adversos mais comuns, sem alopecia significativa.

## Efeitos Adversos

Fadiga (~50%), náusea (~40–50%), anemia ([[Grau Histológico (G1–G3)|G3]] ~10–20%) e aumento de creatinina sérica (por inibição tubular, não nefrotoxicidade). Eventos menos frequentes incluem trombocitopenia, aumento de transaminases, e raramente pneumonite ou síndrome mielodisplásica/leucemia mieloide aguda (<1,5% com uso prolongado). A tolerabilidade é geralmente superior à quimioterapia.
