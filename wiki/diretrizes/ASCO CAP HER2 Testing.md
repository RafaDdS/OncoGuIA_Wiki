---
title: "ASCO/CAP HER2 Testing"
aliases:
  - "ASCO CAP HER2 Guideline"
  - "HER2 Testing Guideline 2018"
  - "Diretriz ASCO/CAP HER2"
category: "Diretrizes e Instituições"
source_documents:
  - "Estadiamento"
tags:
  - "diretriz"
  - "biomarcador"
  - "patologia"
status: "draft"
---

# ASCO/CAP HER2 Testing

**Título original:** Human Epidermal Growth Factor Receptor 2 Testing in Breast Cancer: American Society of Clinical Oncology/College of American Pathologists Clinical Practice Guideline Focused Update

## Informações da Fonte

- **Autores:** Wolff AC; Hammond MEH; Allison KH; Harvey BE; Mangu PB; Bartlett JMS; et al.
- **Ano:** 2018
- **DOI:** [10.1200/JCO.2018.77.8738](https://doi.org/10.1200/JCO.2018.77.8738)
- **Revista:** Journal of Clinical Oncology
- **Detalhes da Publicação:** 36(20):2105-22
- **Documento de Origem:** Estadiamento

## Resumo

Atualização focalizada da diretriz conjunta ASCO/CAP para testagem de HER2 em câncer de mama. Revisa a definição de IHC 2+ (equivoco), torna facultativa a repetição do teste em biópsia versus peça cirúrgica, e estabelece critérios rigorosos para interpretação de padrões ISH incomuns (grupos 2-4), exigindo revisão concomitante com IHC para determinação precisa do status HER2.
